...
-- Fully strip inherited privileges (including anon SELECT) and re-grant SELECT-only to authenticated.
REVOKE ALL ON public.coaching_sessions FROM PUBLIC, anon, authenticated;
REVOKE ALL ON public.coaching_actions  FROM PUBLIC, anon, authenticated;
...
-- Corrected atomic session creator: require ≥1 real action after per-item validation.
...
BEGIN
  ...
  END IF;
  IF length(v_notes) > 4000 THEN v_notes := substring(v_notes from 1 for 4000); END IF;
  ...
  ELSE
    ...
    IF array_length(v_focus, 1) > 8 THEN v_focus := v_focus[1:8]; END IF;
  END IF;
  ...
  -- Require at least one real action; rollback happens automatically because
  ...
  -- Audit metadata intentionally excludes notes/descriptions.
  ...
  VALUES (
    ...
    jsonb_build_object('rep_id', p_rep_id, 'action_count', v_inserted)
  );
...
-- Corrected status updater: reps forward-only, managers full, unified not-found for cross-team.
...
BEGIN
  ...
  v_is_mgr := public.has_management_role(v_actor) AND v_action.team_id = v_actor_team;

  -- Unified response for cross-team / unauthorized to avoid leaking existence.
  IF NOT (v_action.rep_id = v_actor OR v_is_mgr) THEN
    RAISE EXCEPTION 'action_not_found' USING ERRCODE = 'P0002';
  END IF;

  -- Reps: forward-only pipeline. Managers may set any whitelisted status.
  IF NOT v_is_mgr THEN
    IF v_action.status = 'completed' THEN
      RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
    END IF;
    IF NOT (
      (v_action.status = 'assigned'    AND p_status = 'in_progress') OR
      (v_action.status = 'in_progress' AND p_status = 'completed')
    ) THEN
      RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501';
    END IF;
  END IF;
...
-- Execute permissions: authenticated only, never anon/PUBLIC.
